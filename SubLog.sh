...
execute_loop=false
read -p "${PURPLE}[+]${NC}Do you want to scan ports on each subdomain? ${RED}(It will take time)${NC} (y/N): " choice
...
if $execute_loop; then 
    read -p "${CYAN}Enter the ports or range of ports you want to scan separated by comma or dash.${NC} ${RED}(Example. 80,443,8080... or 1-65535. Leave it blank if you want to scan for common ports only.)${NC}: " ports
    echo
    if [-z"$ports"]; then
